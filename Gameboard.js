...
export default class Gameboard {
  ...
  setColorScheme(scheme) {
    ...
    this.cycleGrid(cell => {
      cell.colorScheme = scheme;
    });
    this.drawGrid();
  }

  clearCanvas() {
    this.context.fillStyle = this.colorScheme.board;
  ...
  setupGrid(resolution, state, liveSpots, deadSpots) {
    ...
    if (Array.isArray(liveSpots)) {
      liveSpots.forEach(spot => {
        this.grid[spot[0]][spot[1]].resurrect();
      });
    }
  ...
  drawGrid() {
    this.cycleGrid(cell => {
      this.context.fillStyle = cell.getColor() || this.colorScheme.board;
      this.context.fillRect(
        cell.col * this.resolution,
        cell.row * this.resolution,
        this.resolution,
        this.resolution
      );
    });
  ...
  getLiveNeighborCount(cell) {
    ...
    this.context.fillText(
      `${cell.col}x${cell.row}: ${liveCount}`,
      cell.col * this.resolution + 15,
      cell.row * this.resolution + 50
    );
    return liveCount;
  }
